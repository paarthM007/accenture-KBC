# businessintelligence.ai — Pipeline Contract v1
...
§5.3 has been filled in from the actual `ml_engine` source. Read it before building the prescription and case-matching logic — it answers what `correlated_anomalies` means, what the full `context_tags` vocabulary is, and where baselines come from.
...
- **Three processing stages, not four.** Prescription, case matching and narrative generation all live inside C3 as one module with one entry point. C2 calls C3 exactly once.
...
### 6.3 Refusal handling — MANDATORY
...
**No LLM call. No prescriptions. No invented explanation.** C2 renders the refusal directly from `anomaly_report.refusal`.

This is a flagship demo scenario, not an edge case. It must be explicitly implemented and tested, not left to fall out of the happy path.
